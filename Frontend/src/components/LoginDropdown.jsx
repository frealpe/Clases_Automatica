...
export default function LoginDropdown({ isOpen, onClose }) {
  ...
  // Modo de vista: 'login', 'registro', 'recuperar', 'cambiar_auth'
  ...
  const [pasoRecuperar, setPasoRecuperar] = useState(1); // 1: Pedir Correo, 2: Pedir Código y Nueva Contraseña
  ...
  return (
    <div className={`absolute right-0 top-14 w-[90vw] sm:w-[400px] max-w-[420px] border backdrop-blur-2xl rounded-2xl p-5 shadow-2xl z-50 transition-all animate-fadeIn ${
      ...
      <div className={`flex justify-between items-center mb-3 pb-2 border-b ${
        ...
        <div className="flex items-center gap-2">
          <span className={`material-symbols-outlined text-xl ${esLight ? 'text-sky-600' : 'text-[#38bdf8]'}`}>
            {modo === 'login' ? 'lock' : modo === 'registro' ? 'person_add' : modo === 'recuperar' ? 'mark_email_unread' : 'key'}
          </span>
      ...
      {/* Pestañas Modo Login / Registro cuando no está en recuperar */}
      {modo !== 'recuperar' && modo !== 'cambiar_auth' && (
      ...
      {/* 3. FORMULARIO DE CAMBIO DE CONTRASEÑA (USUARIOS AUTENTICADOS) */}
      {modo === 'cambiar_auth' && (
      ...
      {/* 4. FORMULARIO DE RECUPERACIÓN DE CONTRASEÑA (POR CORREO) */}
      {modo === 'recuperar' && (
