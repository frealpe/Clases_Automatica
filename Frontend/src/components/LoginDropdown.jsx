...
export default function LoginDropdown({ isOpen, onClose }) {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setEsError(false);
        // Ruteo inmediato a /labor (vista inicial de Mis Materias) con React Router
        navigate('/labor');
        setTimeout(() => {
          onClose();
        }, 400);
  ...
  return (
    <div className={`absolute right-0 top-14 w-80 border backdrop-blur-2xl rounded-2xl p-5 shadow-2xl z-50 transition-all animate-fadeIn ${
      esLight
